		...
		<nav class="navbar navbar-expand-lg">
						...
						<div class="col-md-8">
							<form:form action="blog-search" class="search-form" method="GET">
								<div class="form-group">
									<input type="search" placeholder="What are you looking for?" name="query" />
			...
			<div class="container">
				...
				<div
					...
					<!-- Navbar Brand -->
					<a href="${pageContext.request.contextPath}/index-page" class="navbar-brand">Bootstrap Blog</a>
					<!-- Toggle Button-->
						...
						<c:forEach var="result" items="${searchResults}">
							...
							<div class="post col-xl-6">
								<div class="post-thumbnail">
									<a href="blog-post?blogName=${result.seoName}">
									<c:if test="${not empty result.images}">
								...
								<div class="post-details">
									<div class="post-meta d-flex justify-content-between">
										<div class="date meta-last">${result.formattedDateMonth}</div>
										<div class="category">
										<c:choose>
											<c:when test="${result.category.name ne null}">
									<a href="blog-category?categoryName=${result.category.seoName}">
                                    ...
                                    <a href="blog-author?authorUrl=${result.user.seoName}"
											class="author d-flex align-items-center flex-wrap">
										<div class="avatar">
										<img src="${result.user.image.imageUrl}" alt="Author Photo"
													class="img-fluid rounded-circle"
													style="width: 100%; height: 100%; object-fit: cover;">
													</div>
										<div class="title">
											<span>${result.user.name} ${result.user.surname}</span>
										</div></a>
										<div class="date">
											<i class="icon-clock"></i>${result.formattedDateMA}</div>
										<div class="comments meta-last">
											<i class="icon-comment"></i>${result.comments.size()}</div>
											</footer>
									</div>
								</div>
						</c:forEach>
